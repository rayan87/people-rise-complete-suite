...
/// Grade and SalaryBand are both core entities in the same DbContext, so this is a single joined
/// query (no cross-context stitching needed).</summary>
internal static class SalaryBandProjections
{
    public static Task<List<SalaryBandRowDto>> RowsAsync(CoreDbContext db, CancellationToken ct) =>
        Project(db, db.Grades.OrderBy(g => g.Rank)).ToListAsync(ct);

    public static Task<SalaryBandRowDto?> RowForGradeAsync(CoreDbContext db, Guid gradeId, CancellationToken ct) =>
    ...
    public static async Task<decimal?> PreviousMidpointAsync(CoreDbContext db, int rank, CancellationToken ct)
    {
    ...
    public static async Task CascadeMidpointsAsync(CoreDbContext db, int editedRank, decimal newMidpoint, CancellationToken ct)
    {
    ...
    private static IQueryable<SalaryBandRowDto> Project(CoreDbContext db, IQueryable<Grade> grades) =>
        grades.Select(g => new SalaryBandRowDto(
            ...
            db.SalaryBands.Where(b => b.GradeId == g.Id && b.JobFamilyId == null)
                .Select(b => new SalaryBandInfo(
                    ...
                    b.HalfSpreadPct, b.MinAmount == 0 ? 0m : (b.MaxAmount / b.MinAmount - 1m) * 100m,
                    b.OverlapPct, b.EffectiveDate, b.Status.ToString(), b.Provenance.ToString(), b.Basis.ToString()))
                .FirstOrDefault()));
}
